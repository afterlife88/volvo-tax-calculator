...
public class CongestionTaxCalculatorTests
{
    ...
    public async Task GetTaxReturnsCorrectAmountByTheRules(int expected, params string[] passes)
    {
    ...
    public async Task GetTaxReturnsZeroForExemptVehicles(Type vehicle)
    {
        ...
        var taxCalculator = new CongestionTaxCalculator();
        var tax = await taxCalculator.GetTax(CityToTest, (IVehicle) Activator.CreateInstance(vehicle)!, new List<DateTime>() {DateTime.Now});
        Assert.Equal(0, tax);
    }

    [Fact]
    public async Task GetTaxThrowsArgumentExceptionWhenRequestedForCityWithoutRules()
    {
        var taxCalculator = new CongestionTaxCalculator();
        await
